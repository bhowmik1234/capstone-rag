...
async function runComparison() {
    ...
    console.table(results.map(r => ({
        Model: r.model,
        AvgLatency: (r.queries.reduce((acc, q) => acc + (q.duration || 0), 0) / r.queries.length).toFixed(2) + 's',
        Status: r.queries.every((q) => !q.error) ? '✅ Success' : '❌ Errors'
    })));
